...
This repo is the **all-inclusive Pepper sandbox**: bridge + Studio UI + the AI brain, in one package. The **lean** build ships **no model weights and no inference binary** — you bring a GGUF and a `llama-server`. (Historically the brain lived in a separate `pepper-ai` repo; that brain now merges in here — see "Relationship to pepper-ai".)

> **Decision (2026-06-17) — `full` build reverses the no-auto-download pillar.** There are now TWO packaged builds. **lean** (default) is unchanged in spirit: full app incl. voice, bring-your-own LLM, no auto-download. **full** is for users who don't run local LLMs: on first run it auto-downloads a *prebuilt* llama.cpp server binary (no compiling) + a recommended GGUF, then hands off to the existing `runner.py` sidecar. The original "nines-breaking" rejection was about (a) bloating the bundle with weights/binaries and (b) cross-platform binary resolution. (a) is preserved — `full` downloads at *runtime*, the zip stays lean. (b) is contained by defaulting GPU to **Vulkan** (cross-vendor, the only GPU prebuilt llama.cpp ships for Linux), with **CUDA as a Windows-only manual override** and **Metal** auto on Apple Silicon. Logic + cross-platform asset resolution live in `provision.py`, unit-tested offline against the real release manifest. The marker `bundle.json` (written by the `.spec`) is the only code-level difference between builds.
...
3. ✅ **AI brain + code-testing affordances** *(DONE & verified — merged thesis)* — split into focused specs:
   - ✅ **3a — AI provider config** *(DONE & verified)*: runtime-settable `/ai/config` (cloud key + local URL), persisted to `~/.pepper-studio/ai.json` (`0600`, atomic write); `AISettings` UI panel; key never returned to the browser (redacted from the API log too).
   - ✅ **3b — Local model runner** *(DONE & verified)*: pick a GGUF → launch a user-provided `llama-server` **sidecar** (`runner.py`; detect on PATH or point at the binary) → `/health`-poll → stream stdout to an in-app log → clean kill on exit (atexit/SIGTERM); common flags + raw-args escape hatch; runner-over-dial precedence (`_rebuild_brain`, never persisted). Beginner guide is `LLAMA_SETUP.md` at repo root. **No auto-compile, no auto-download** — rejected as nines-breaking.
   - ✅ **3c — Code-testing affordances** *(DONE & verified)*: in-app API reference popup (`ApiReference` + `api_catalog.json` + `snippets.js`, curl/Python copy with bridge URL substituted) opened from the Dashboard; a pytest drift test asserts every catalog path resolves on a live bridge; the live API log is preserved.
4. ✅ **Package** *(DONE & verified)* — PyInstaller **onedir** "localhost app" per OS (`pepper-studio.spec`): bundles `web/dist` via a `sys._MEIPASS`-aware `DIST_DIR`; `opencv`/`pyaudio`/`numpy`/`tkinter` excluded so the base build stays lean (~26 MB). Cross-platform **CI matrix** (`.github/workflows/build.yml`, ubuntu/macos/windows — PyInstaller can't cross-compile, so each OS builds on its own runner; rebuilds the web UI, pins Python 3.12) that **builds + smoke-tests** (launch frozen binary → `GET /ai/config` returns 200) on all three and uploads a per-OS artifact. Linux artifact also verified locally from a clean dir (no source tree/venv): UI/asset/`/ai/config`/WS all respond. Optional pywebview native-window build **deferred**.

**Deferred sub-projects (own specs later):** ⬅️ SP2 — real-robot bridge mode (teleop, live joint mirroring, **safety: e-stop, motion limits, connection-loss handling**). SP3 — sim→real one-command deploy/export. Optional: pywebview native-window build; macOS code-signing / Windows signing to avoid Gatekeeper/SmartScreen on downloaded artifacts.
