...
3. **AI brain + code-testing affordances** *(IN PROGRESS — merged thesis)* — split into focused specs:
   ...
   - ⬅️ **3c — Code-testing affordances** *(NEXT)*: in-app API reference + copy-paste curl/Python snippets (frontend catalog + a drift test asserting every catalog path exists in the bridge route table); preserve the existing live API log (already wired: `log_api_call` → WS → Dashboard).
